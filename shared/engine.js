...
(function() {
  ...
  // Patch header/footer labels
  document.querySelectorAll('[data-header-label]').forEach(el => el.textContent = cfg.headerLabel);
  document.querySelectorAll('[data-footer-label]').forEach(el => {
    el.textContent = '© 2026 Netscribes, Inc. · ' + cfg.footerLabel;
  });
  ...
  if (introCard) {
    introCard.innerHTML = `
      ...
      <button class="btn-submit" onclick="startAssessment()">Start the benchmark →</button>`;
  }

  // Build 5 question screens from flat 10-question list
  ...
  const scaleOpts = [
    {v:1,lbl:"Not in place"},{v:2,lbl:"Early stages"},
    {v:3,lbl:"In progress"},{v:4,lbl:"Well established"},{v:5,lbl:"Fully embedded"},
  ];
  ...
  const stages = [
    ...
    {n:3,name:"Scaling",   color:"#005F86",bg:"#E8F4F8",desc:"AI is working in specific areas. The challenge is systematic expansion and embedding AI across the business with consistent governance and operational discipline."},
  ...
  window.startAssessment = function() {
    cur = 0; answers = new Array(10).fill(0);
    renderScreen(); show('s-questions');
  };
  ...
  function renderScreen() {
    ...
    s.questions.forEach((q,qi) => {
      ...
      card.innerHTML += `
        <div class="question-block" id="qblock-${qi}" style="${isLast?'border-bottom:none;padding-bottom:0;margin-bottom:24px':''}">
    ...
    card.innerHTML += `
      <div class="nav">
        ...
        <button class="btn-next" id="btn-next" onclick="nextScreen()" ${bothAnswered?'':'disabled'}>
  ...
  window.nextScreen = function() { if (cur < 4) { cur++; renderScreen(); } else { show('s-gate'); } };
  window.prevScreen = function() { if (cur > 0) { cur--; renderScreen(); } };
  ...
  window.submitGate = function() {
    ...
    // Normalise all dims to /10 for display (single-Q dims ×2)
    ...
    sb.innerHTML = `
      ...
      <div class="stage-score" style="color:#888">Score: ${total} / 50</div>
      <div class="stage-desc-text" style="color:#555">${stage.desc}</div>`;

    // Dim bars using industry-specific dim names
    ...
    // Top 2 gaps — industry-specific messages
